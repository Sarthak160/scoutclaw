...
export async function extractResumeInsights(resumePath: string): Promise<Insights> {
  if (!resumePath) {
...
function emptyResumeInsights(): Insights {
  return {
    excerpt: "",
    searchSignals: []
  };
...
export function deriveSearchSignals(text: string): string[] {
