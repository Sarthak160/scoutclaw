...
import { createCheckoutResponse, type CheckoutSessionParams } from "../app/api/checkout/route.js";
...
test("checkout creates a stripe session with expected urls and amount", async () => {
  ...
  const capture: { key: string; payload: CheckoutSessionParams | null } = { key: "", payload: null };
  const response = await createCheckoutResponse(
    new Request("http://localhost/api/checkout", {
      method: "POST",
  ...
  assert.equal(capture.key, "sk_test_123");
  assert.ok(capture.payload);
  const payload = capture.payload;
  assert.equal(payload.line_items[0].price_data.unit_amount, 2000);
  assert.equal(payload.success_url, "http://localhost:3001/success?session_id={CHECKOUT_SESSION_ID}");
